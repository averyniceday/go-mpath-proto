...
if [ ! -f ${SWAGGER_CODEGEN_CLI_JAR} ]; then
    if which curl ; then
        curl 'https://repo1.maven.org/maven2/org/openapitools/openapi-generator-cli/6.2.1/openapi-generator-cli-6.2.1.jar' --output ${SWAGGER_CODEGEN_CLI_JAR}
    else
        if which wget ; then
            wget 'https://repo1.maven.org/maven2/org/openapitools/openapi-generator-cli/6.2.1/openapi-generator-cli-6.2.1.jar' -O ${SWAGGER_CODEGEN_CLI_JAR}
        else
